...
def predict_price(date_str):
    ...
    try:
        ...
        print(f"Predicted Open price: {inverse_prices[0][0]:.2f}")
        print(f"Predicted Close price: {inverse_prices[0][1]:.2f}")
        ...
        end_time = time.time()
        print(f"Prediction execution time: {end_time - start_time:.2f} seconds")

    except KeyError:
        print(f"No actual prices available for the date: {date_str}.")

# Example usage
input_date = '2023-12-01'  # Change this date to any date in 2023

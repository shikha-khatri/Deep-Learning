...
# Training function for Ray with sparsity
@ray.remote
def train_model_with_sparsity(X_train, y_train, num_epochs):
    model.train()
    ...
    # Fine-tune the model after applying sparsity
    for epoch in range(50):  # Reduced epochs for fine-tuning
        optimizer.zero_grad()
        outputs = model(torch.FloatTensor(X_train))
        loss = criterion(outputs, torch.FloatTensor(y_train))
        loss.backward()
...
# Reduce parallelism for Ray by limiting to 2 workers
num_workers = 2
futures = [train_model_with_sparsity.remote(X_train, y_train, 50) for _ in range(num_workers)]
...
torch.save(models_state[-1], 'stock_price_model_ray_sparse.pth')

...
model.load_state_dict(torch.load('stock_price_model_ray_sparse.pth'))
model.eval()
...
def predict_price(date_str):
    ...
    try:
        ...
        # Print predicted Open and Close prices
        print(f"Predicted Open price for {date_str}: {inverse_prices[0][0]:.2f}")
        print(f"Predicted Close price for {date_str}: {inverse_prices[0][1]:.2f}")
